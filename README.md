...
rules:
  ...
  - **雷达（按档位路径激活 · WS 实时）**：`REGIME_RADAR` — R1 **50%**/步进35%/呼吸1.0ATR · R2 **60%**/30%/0.8 · R3 **70%**/25%/0.65 · R4 **80%**/20%/0.5；**价格源 = markPrice WebSocket**（`ws_price_listeners`）；紧 TP1 有效比例抬高；开仓 25s + 双轮确认；TP 成交强制激活；误挂 `RADAR_REVOKE`；与 TP123/TV硬止损互不抢份额
...
| 路径雷达 + WS 实时 + 档位呼吸 | 🟡 P1 | R1–R4：50/60/70/80% · move_step · ATR 呼吸 · markPrice WS · 25s + 双确认 |
...
- [x] **路径比例雷达 + 阶段锁利**（R1–R4：50/60/70/80% · 步进 35/30/25/20% · 呼吸 1.0/0.8/0.65/0.5 ATR）+ 紧 TP1 有效比例 / 开仓保护期 / 双轮确认 / `RADAR_REVOKE`
