...
> **文档同步（2026-07-21）：**  
> 铁律：**TV → 先平后开 → LIMIT@`price` 开仓（不足市价补）→ 挂 TP123+硬止损各一次 → 雷达候命 → 钉钉确认**。  
> 仓位 = TV `risk_pct`/`qty_ratio`/`leverage`（已删 REGIME_MARGIN / 50000 硬顶）；硬止损 = 严格 `tv_sl`；雷达 R1–R4 **50/60/70/80%**（挂上后**只前进不撤**）；钉钉显示 **TV 杠杆** + 挂载确认（硬止损/TP123/雷达候命）。  
> 自查清单：[§VPS 实盘自查清单](#vps-实盘自查清单) · [`docs/VPS_LIVE_CHECKLIST.md`](docs/VPS_LIVE_CHECKLIST.md) · [§实盘事故](#实盘事故--注意事项--优化指南)。
...
trading_factory:
  ...
  defense_route_a: |
    ...
    先平后开清场时必须保留/恢复 tv_sl（keep_tv_sl），否则算仓会 missing_tv_sl；
    UPDATE_SL → 按新 tv_sl 改挂；雷达达档位路径比例后启动保本追踪（可改条件单槽）；
    DeepCoin 双轨条件单并行
  entry_order: |
    LIMIT @ TV price（Binance/OKX/Gate=IOC；DeepCoin=GTC 短等后撤未成部分）；不足额 → 市价补余量；
    实现：_place_tv_entry_order（两 supervisor）
  leverage: |
    开仓/加仓：set_leverage(TV leverage) + 算仓用 TV leverage；钉钉 OPEN 显示 TV 杠杆与等效杠杆；
    config LEVERAGE/OKX/GATE/DEEPCOIN=25 仅作缺省回退（勿当实盘杠杆）
  sizing: |
    唯一公式：止损距离=|price-tv_sl|；风险金额=权益×(risk_pct/100)；理论仓位=风险/距离；
    杠杆限制=权益×leverage/price；最终=min(理论仓位, 杠杆限制)×qty_ratio（已删 50000 硬顶）；floor 到品种步长；
    OPEN/ADD 同一公式；VPS 禁止自算 risk_pct/qty_ratio/tv_sl/tp；禁止 REGIME_MARGIN；
    CAP 亦用同一 TV risk 公式（非 margin×25×）；硬止损挂失败 → 立即撤仓（禁止裸奔）
  webhook_order: |
...
rules:
  ...
  - **开仓单**：优先 **LIMIT @ TV `price`**，未足额再市价补（禁止一上来纯市价当唯一路径）
  - 开仓后挂 **基础单×3**：TP1/2/3（reduceOnly，各挂一次）+ **TV硬止损条件单**（价=`tv_sl`，只挂一次；禁止普通限价秒平）
  - **硬止损**：严格按 TV `tv_sl`；缺 `tv_sl` → 告警/拒开或撤仓，**不**用宽止损兜底；挂失败 → 立即撤仓禁裸奔
  - **雷达（按档位路径激活 · WS 实时）**：`REGIME_RADAR` — R1 **50%** · R2 **60%** · R3 **70%** · R4 **80%**（步进 35/30/25/20% · 呼吸 1.0/0.8/0.65/0.5 ATR）；**价格源 = markPrice WS**；紧 TP1 有效比例抬高；开仓 25s + 双轮确认；TP 成交强制激活；**挂上后只前进、禁止解除**（`_clear_premature_radar_arm` = no-op）；与 TP123/硬止损互不抢份额
  - **钉钉**：实盘核查后推一次；OPEN 含 TV 杠杆、等效杠杆、硬止损/TP123/雷达候命 ✅；`dingtalk_alert_dedupe` 去重
  - 禁止与 TV 反向持仓：哨兵 / 重启 / 空闲巡检 → FORCE_ALIGN 全平
  - 人工/外部同向仓：manual adopt 后保留仓位，TV CLOSE 不强制全平，补挂 TP123 + **TV硬止损**（雷达按档位路径比例激活）
  - **人工/外部全平**：哨兵/空闲巡检检测到实盘归零 → **立即撤销 TP123 + 硬止损/STOP**，防止孤儿止盈反向开仓
  - **雷达越过 TP1/TP2**：雷达 SL 越过对应 TP → `TP_ORPHAN_PURGE` 撤过时限价
  - **加仓合并**：PYRAMID/PROFIT_ADD → 加权均价重置雷达、硬止损沿用/更新 `tv_sl`、新 TP123 替换旧限价
  - **三层止损**：`CLOSE_STOPLOSS` 市价全平 > 盘口 `tv_sl` > 雷达动态锁利
  - 未结清绩效 / 用户暂停 / 全局暂停 → 跳过建仓；平仓类信号暂停时仍放行
...
└────────────────────────────┬─────────────────────────────────────┘
  ...
  TradeLog      全域日志       DownlineLogsModal
                   │
                   ▼
         钉钉（按交易所 GEMINI 主题 `#币安{lev}x·ETH` / `#深币…` / `#OKX…` / `#Gate…`，lev=TV 杠杆优先，仅关键动作）
```
...
| **VPS 独立 sizing** | 唯一公式：权益×risk_pct/\|price−tv_sl\|，再与杠杆限制取 min × qty_ratio（**无** 50k 硬顶）；硬止损 = TV `tv_sl` |
...
TradingView POST /gemini/webhook
  ...
  → set_leverage(TV leverage) → **LIMIT@TV price 开仓**（不足市价补）→ 挂 TP123 + TV硬止损(必须成功否则撤仓) → 雷达候命 → 钉钉确认
```
...
`risk_multiplier = global_risk × 用户档位`（0.6 / 1.0 / 1.4）注入 payload，用于 **cap guard**（CAP 亦走 TV risk 公式，非 REGIME_MARGIN）。
...
| 场景 | 四所行为 |
|------|----------|
| 空仓 | TV risk 公式算仓 → **LIMIT@`price`**（不足市价补）→ **TP123 限价 + TV硬止损** + 哨兵（雷达候命） |
| 已有仓 · **工厂单** | **先平后开** |
...
算账示例（本金 1000 U · ETH@1892.43 · qty_ratio=1 · **TV leverage 以策略下发为准**，下表为 risk 公式结果）：

| Regime | risk_pct | 止损距离 | 下单量 | 名义约 | 等效杠杆约 |
|--------|----------|----------|--------|--------|------------|
| R1 | 0.81% | 12.08 | **0.67 ETH** | 1,268 U | 1.27× |
| R2 | 1.35% | 14.09 | **0.96 ETH** | 1,817 U | 1.82× |
| R3 | 2.03% | 14.02 | **1.45 ETH** | 2,744 U | 2.74× |
| R4 | 2.70~3.38% | 15.94 | **1.69~2.12 ETH** | 3.2k~4.0k U | 3.2~4.0× |

换算：任意本金下单量 ≈ 上表(1000U) × (本金/1000)。加仓 `qty_ratio=0.3~0.7` 按比例缩小。硬止损挂单价 = TV `tv_sl`（权威）。
...
|------|-------------------|----------------------|---------------------|------|
| R1 极弱 | **50%** | **35%** | **1.0 ATR** | 保本监控 |
| R2 弱势 | **60%** | **30%** | **0.8 ATR** | 保本监控 |
| R3 中势 | **70%** | **25%** | **0.65 ATR** | 保本+移动止损 |
| R4 强势 | **80%** | **20%** | **0.5 ATR** | 保本+移动+追踪 |
...
1. **有效激活**：若 `|TP1−entry| < 1×ATR`，有效路径抬至约 **85%**（再叠加绝对位移地板：`max(0.55×ATR, 0.15%×entry)`）
2. **开仓保护期**：`trade_opened_at` 后 **25s** 内禁止路径启动
3. **双轮确认**：连续 **2** 次确认均达有效比例才 `_latch_radar`
4. **只前进不撤**：雷达 latched 后**禁止解除**；`_clear_premature_radar_arm` 为 no-op（历史 `RADAR_REVOKE` 文案保留，不再撤雷达）
...
7. **防秒挂秒撤**：盘口已对齐 → `live_already_aligned` / `on_book` 跳过；WS 与哨兵共用锁，禁止新旧雷达/硬止损双系统抢权限
...
| Regime | OPEN 说明 | TP 分批 (%) | 雷达激活 | 步进 | 呼吸 ATR× |
|--------|-----------|-------------|----------|------|-----------|
| 1 | TV `risk_pct`（约0.81%） | **25/35/40** | **50%** | **35%** | **1.00** |
| 2 | TV `risk_pct`（约1.35%） | 20/35/45 | **60%** | **30%** | **0.80** |
| 3 | TV `risk_pct`（约2.03%） | 18/32/50 | **70%** | **25%** | **0.65** |
| 4 | TV `risk_pct`（约2.70~3.38%） | 5/20/75 | **80%** | **20%** | **0.50** |

> 开仓量**只**用 TV `risk_pct`/`qty_ratio`/`leverage`；雷达参数唯一源码：`radar_trail.py` → `REGIME_RADAR`。
...
| 交易所 | 标签 | 主题色 | 品牌 |
|--------|------|--------|------|
| Binance | `#币安{TV杠杆}x·ETH`（缺省回退读 `LEVERAGE`） | 靛蓝 🔷 | GEMINI量化 · 币安合约实盘引擎 |
| DeepCoin | `#深币{TV杠杆}x·ETH` | 翡翠绿 🟢 | GEMINI量化 · 深币 SWAP 实盘引擎 |
| OKX | `#OKX{TV杠杆}x·ETH` | 紫罗兰 🟣 | GEMINI量化 · OKX 合约实盘引擎 |
| Gate | `#Gate{TV杠杆}x·ETH` | 琥珀橙 🟠 | GEMINI量化 · Gate 合约实盘引擎 |

> 标题里的 **×杠杆** 优先取本笔 `detail.leverage`（TV 下发），**禁止**把 config 25× 当成实盘杠杆展示。XAU 标签后缀为 `·XAU`。

前端 API 绑定页交易所卡片使用同色主题（`exchange-picker-{exchange}`），便于与原版系统视觉区分。

**会推送：** `OPEN`、`PYRAMID`、`PROFIT_ADD`、`NOTIONAL_CAP`、`CLOSE`、`CLOSE_TP3`、`CLOSE_PROTECT`、`CLOSE_STOPLOSS`、`CLOSE_ATTRIBUTION`、`POSITION_RECONCILE`、`RADAR_ARM`、`TRAIL`、`SAME_DIR_TP_REFRESH`、`SAME_DIR_REOPEN`、`STARTUP`、`STARTUP_FAIL`、`FORCE_ALIGN`、`POSITION_SIDE_FLIP`、`TP_OVER_COMMIT`、`ADJUST`、`MANUAL_ADJUST`、`DEFENSE_HEAL_FAIL`、`INSUFFICIENT_BALANCE`、`LOCK_TIMEOUT`、`TP_RETRY_FAIL`、`API_OFFLINE`、`SENTINEL_ERROR`、`severity=critical`

> `RADAR_REVOKE` 类型名保留兼容，实盘逻辑已改为**禁止解除**；不应再出现「雷达解除」类钉钉。

**平仓来源区分（钉钉标题/核实明细）：**

| 场景 | 钉钉识别 |
|------|----------|
| 路径达档位激活%（R1 **50%** / R2 **60%** / R3 **70%** / R4 **80%**）首次挂保本 | `RADAR_ARM`「雷达启动」· 启动来源=路径（每笔一次） |
| TP1/2/3 限价吃单后强制保本 | `RADAR_ARM`「…后防回吐（止盈成交）」· 启动来源=TP成交 |
...
| 字段 | 含义 |
|------|------|
| 方向 / Regime / 开仓价 / 下单量 | 本笔实盘核实 |
| TV杠杆 / 等效杠杆 | TV `leverage`；名义/权益 |
| TV硬止损 | `@价 = tv_sl`（已挂单） |
| TP1/2/3 | 价格 + 分批比例 |
| 盘口结构 | 基础单×3：TP123 + 条件委托×1：TV硬止损 |
| **硬止损已挂载** | ✅ / ❌ |
| **TP1/TP2/TP3已挂载** | ✅ / ❌ |
| **雷达候命** | ✅（开仓时未激活，监控已启动；达路径% 后才 `RADAR_ARM`） |
...
| `TRAIL` | 雷达推止损 |
| `RADAR_ARM` | 雷达按路径比例启动保本追踪（只前进） |
| `RADAR_REVOKE` | **已废弃行为**：历史「误挂解除」；现为 no-op，不应再推解除文案 |
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `SYMBOL` / `TRADING_SYMBOLS` | ETHUSDT / ETH,XAU | 交易对 |
| `LEVERAGE` / `OKX_*` / `GATE_*` / `DEEPCOIN_*` | **25** | **仅缺省回退**；实盘优先 TV `leverage` |
| `SIZING_MARGIN_LEVERAGE` | 5 | 遗留系数（live OPEN 不走 REGIME_MARGIN） |
| `REGIME_MARGIN_1~4` / `VPS_RISK_PCT` | **0（已废弃）** | live 忽略；仓位只认 TV `risk_pct` |
| `ADD_RATIO_REG1~4` | 0 / 0.3 / 0.5 / 0.7 | TV 未传 qty_ratio 时加仓回退 |
| `MAX_ADD_TIMES_REG1~4` | 1 / 2 / 2 / 3 | 档位最大加仓次数 |
| `MAX_COMBINED_NOTIONAL_MULT` | 13.0 | ETH+XAU 合计名义 ≤ 权益×倍率 |
...
| 雷达过早启动 / 近入场误标 TP1 | 查 `evaluate_radar_arm_gate`、有效比例、平仓归因；**不应**再依赖 `RADAR_REVOKE` 撤雷达 |
| 钉钉开仓仍写 25× | 查本笔 `detail.leverage` / `_bind_tv_leverage`；标题应跟 TV |
| OPEN `missing_tv_sl` | 查清场是否抹掉 tv_sl；应 `keep_tv_sl` / `_pending_open_tv_sl` |
| 开仓全是市价无 LIMIT | 查版本 ≥ `38811b4`；日志应有 `LIMIT@` / `entry_order_style` |
...
| 路径雷达 + WS 实时 + 档位呼吸 | 🟡 P1 | R1–R4：**50/60/70/80%** · 只前进不撤 · markPrice WS · 25s + 双确认 |
| 开仓单类型 | 🔴 P0 | LIMIT@TV `price` → 不足市价补；`_place_tv_entry_order` |
| 钉钉 | 🟢 P2 | TV 杠杆 · 挂载确认（硬止损/TP123/雷达候命）· 动作去重 |

自动化验收（`backend/`，Windows 用 `py`）：

```bash
py -m pytest tests/test_dual_symbol.py tests/test_vps_dev_checklist.py tests/test_radar_trail.py \
  tests/test_vps_radar_stages.py tests/test_ws_radar_tick.py tests/test_vps_hard_sl.py \
  tests/test_adverse_radar_guard.py tests/test_close_attribution.py tests/test_trading_alerts.py \
  tests/test_trading_alerts_tp_ratio.py tests/test_vps_entry_routing.py tests/test_webhook_seq.py \
...
- [x] **TV硬止损 Stop-Limit**：严格按 TV `tv_sl`（多空·全所）；条件委托挂单；禁止开仓价×%旧宽止损；禁止普通限价（会秒平）
- [x] **LIMIT@TV price 开仓**：IOC（深币 GTC 短等）不足则市价补；四所统一 `_place_tv_entry_order`
- [x] **路径比例雷达 + 阶段锁利**（R1–R4：50/60/70/80%）+ 紧 TP1 有效比例 / 开仓保护期 / 双轮确认 / **只前进不撤**
...
- [x] **TV 同 bar 先平后开** + 清场保留 `tv_sl`；空仓 OPEN 雷达候命
- [x] **钉钉 OPEN**：TV 杠杆 + 等效杠杆 + 硬止损/TP123/雷达候命挂载确认
...
| 5 | **开仓单** | LIMIT@TV `price`，不足市价补 | 一上来纯市价当唯一路径（滑点不可控） |
| 6 | **钉钉** | 动作实盘核查后推送**一次**；OPEN 含挂载确认 + TV 杠杆 | 监控循环刷屏；标题写死 25× |
| 7 | **四所一套逻辑** | Binance / OKX / Gate / DeepCoin 同一铁律 | 某所仍走旧雷达/旧宽止损/旧 REGIME_MARGIN |
...
| **Commit** | `ce2d3c9` |
| **档位表** | R1 50%/35%/1.0ATR · R2 60%/30%/0.8 · R3 70%/25%/0.65 · R4 80%/20%/0.5（自查清单§五） |

#### H. 雷达噪声误挂 / 平仓误标 TP1（P1）

| 项 | 内容 |
|----|------|
| **现象** | 刚开仓噪声触发保本；或平仓归因把保本止损标成 TP1 |
| **修复** | 25s 开仓保护期 + 双轮确认 + 紧 TP1 有效比例抬高；平仓归因优先 `exchange_stop`；**雷达挂上后禁止解除**（revoke=no-op） |
| **注意** | 雷达哲学是「适度追随」不是「紧追」；勿把 move_step 改回固定 50% 紧跟；勿再启用路径回撤撤雷达 |
...
#### J. 先平后开清场抹掉 `tv_sl` → OPEN 算仓失败（P0 · 2026-07-20）

| 项 | 内容 |
|----|------|
| **现象** | TV 已带 `tv_sl`，钉钉却 `missing_tv_sl` / `DISPATCH_PARTIAL_FAIL` |
| **根因** | `_force_flat_before_open` 调 `_reset_adverse_radar(keep_tv_sl=False)` 清掉本笔 `tv_sl` |
| **修复** | 清场 `keep_tv_sl=True`；`_pending_open_tv_sl` 开仓前恢复；Commit `d9aca26` |

#### K. 钉钉/实盘仍显示 config 25× + REGIME_MARGIN CAP（P0 · 2026-07-20）

| 项 | 内容 |
|----|------|
| **现象** | TV 发 5×，钉钉标题仍 25×；CAP 按旧保证金系数削仓 |
| **根因** | 主题 leverage 回退 config；CAP 仍用 `REGIME_MARGIN × leverage` |
| **修复** | `_bind_tv_leverage`；CAP=`tv_risk_formula`；OPEN detail 注入 TV leverage / `effective_leverage`；`REGIME_MARGIN_*` 置 0 |

#### L. 雷达「解除」+ 钉钉旧 85/80/75/70（P0 · 2026-07-20）

| 项 | 内容 |
|----|------|
| **现象** | 雷达已激活后又被撤销；钉钉图例写 R1=85%… |
| **修复** | `_clear_premature_radar_arm` no-op；图例读 `REGIME_RADAR` 50/60/70/80；Commit `9bae848` |

#### M. 开仓改为 LIMIT@TV price（P1 · 2026-07-21）

| 项 | 内容 |
|----|------|
| **需求** | 自查清单：开仓 LIMIT，价格=`price` |
| **实现** | `_place_tv_entry_order`：IOC LIMIT（深币 GTC 短等）→ 不足市价补；钉钉增加挂载确认三行；Commit `38811b4` |

---

### 三、日常运维要注意什么

1. **VPS 更新后必须重启** webhook（6010）与 backend，否则仍跑旧 seq 门 / 旧雷达 / 旧市价开仓。  
2. **看日志顺序**：同 bar 刷新应看到 CLOSE release → OPEN release → 开仓钉钉有仓；若先 OPEN 后 CLOSE，立即停机查版本。  
3. **币安盘口对账**：开仓后基础单=3、条件委托=1；钉钉应有「硬止损/TP123已挂载 ✅ · 雷达候命 ✅」。  
4. **Pine 字段**：`bar_index`+`seq` 必填；`symbol` 必填；`tv_sl`/`risk_pct`/`leverage` OPEN 必填。  
5. **不要手改 state.json 的 `consumed_tp_levels` / `radar_latched`**，除非知道在修死亡螺旋。  
6. **禁止**在未跑测试时同时改 `webhook_seq_gate` + 两套 supervisor 的挂单路径。  
7. **人工同向仓**：TV **CLOSE** 仍可走 manual 保护；TV **OPEN** 一律先平后开刷新。  
8. **钉钉限流**：企业机器人 20 条/分钟；依赖攒批+去重，勿再加监控循环推送。  
9. **Windows 本地测**：`python` 可能指向 WindowsApps 空壳（exit 9009）→ 用 **`py -m pytest`**。

**常用日志 grep：**

```bash
docker compose logs -f backend | grep -E \
  "WebhookSeq|same-bar CLOSE|hold OPEN|先平后开|LIMIT@|FLIP_CLEAN|TP_SKIP_REHANG|RADAR_ARM|live_already_aligned|FORCE_ALIGN|missing_tv_sl"
```
...
| 优先级 | 方向 | 说明 |
|--------|------|------|
| P0 | 保持铁律单测绿灯 | `test_webhook_seq` + `test_vps_entry_routing` + `test_vps_hard_sl` |
| P0 | 部署后冒烟 | 同 bar：先发 OPEN seq=1 再发 CLOSE seq=2，确认最终有仓；钉钉 TV 杠杆正确 |
| P1 | WS 健康 | 监控 `markPrice` 断线重连；WS 失效时哨兵 1s 兜底 |
| P1 | LIMIT 成交率 | 统计 IOC 足额率；过低可调等待或加价保护 |
| P2 | 钉钉文案 | 开仓/清场/雷达字段统一（已含挂载确认） |
| P2 | Regime 后台可配 | 激活/步进/呼吸进管理后台，但仍单一源码表 |
| P3 | 可观测性 | 「同 bar CLOSE→OPEN 耗时」「force_flat 失败率」「hard_sl_fail_abort」打点 |
...
py -m pytest tests/test_webhook_seq.py tests/test_vps_entry_routing.py \
  ...
  tests/test_radar_trail.py tests/test_adverse_radar_guard.py \
  tests/test_trading_alerts.py tests/test_position_cap_guard.py -q
```
...
- [ ] 任意 OPEN → 调用 `_force_flat_before_open`（有仓平仓 / 无仓清挂单）；`tv_sl` 不被清场抹掉  
- [ ] 开仓 → LIMIT@TV price（或已记录市价补路径）  
- [ ] 单独 CLOSE → 仓位 0 + 挂单 0 + 雷达/consumed 复位  
- [ ] 雷达读 WS mark；激活/步进/呼吸只读 `REGIME_RADAR` 50/60/70/80；挂上后不撤  
- [ ] TP123 / 硬止损 / 雷达不互相撤挂；对齐则跳过；硬止损失败撤仓  
- [ ] 钉钉：TV 杠杆 + 挂载确认；关键动作去重  
- [ ] 四所行为一致；无 REGIME_MARGIN / 旧宽止损双轨  
- [ ] README / `docs/VPS_LIVE_CHECKLIST.md` 与代码同步；已 push `main`

---

## 更新记录

> 按时间倒序。生产 VPS：`git pull` → `docker compose up -d --build`（或既有部署脚本）后**必须重启** backend（含 webhook 6010）。

### 2026-07-21 · LIMIT@TV price 开仓 + 钉钉挂载确认 · README 全量同步

| Commit | 内容 |
|--------|------|
| `38811b4` | `_place_tv_entry_order`：LIMIT IOC（深币 GTC）→市价补；OPEN 钉钉 `硬止损/TP123已挂载` + `雷达候命` |
| `26a96fd` | 钉钉主题断言对齐 `#…x·ETH`；CAP 明细不再要求「本金快照」 |
| `f426222` | OPEN 钉钉显示 TV leverage + `effective_leverage`；`REGIME_MARGIN_*` 默认归零 |
| `9bae848` | 雷达只前进不撤；钉钉 R1–4 = 50/60/70/80 |
| `bb35732` | 绑定 TV leverage；CAP 改 TV risk 公式 |
| `d9aca26` | 先平后开清场保留 `tv_sl` |

**文档：** 本 README 顶部同步条、Agent 速查、钉钉/环境变量/事故档案/自查清单与上述铁律对齐；`docs/VPS_LIVE_CHECKLIST.md` 同步。
...
### 2026-07-20 · 对齐终极版策略自查清单：TV sizing + TV硬止损 + 雷达50/60/70/80

| 项 | 内容 |
|----|------|
| 仓位 | 唯一公式 TV `risk_pct`/`qty_ratio`/`leverage`；对照表 R1~R4 @1000U 验收 |
| 硬止损 | 删除 VPS 宽止损/`REGIME_HARD_SL_PCT`；触发价=tv_sl 原值 |
| 雷达 | 激活对齐自查清单：R1 **50%** · R2 **60%** · R3 **70%** · R4 **80%**；WS 实时 |
| TP | 成交后绝不补挂已成交档（防 TP1 死亡螺旋） |
| 钉钉 | 同类一次；开仓核实 TP123+硬止损+雷达候命 |
...
| 项 | 内容 |
|----|------|
| 开仓顺序 | 先平后开 → **LIMIT@TV price**（不足市价补）→ **只挂一次** TP123 → **只挂一次** TV `tv_sl` 硬止损 → 雷达候命 → 钉钉确认 |
...
| TV | 有仓/雷达中 → **一律先平后开**（清场保留 tv_sl + 新 TP123 + 硬止损 · 雷达候命）；空仓仅 OPEN → LIMIT 开仓、雷达候命 |
...
### 2026-07-19 · `89e1446` — 档位雷达（历史；已由 50/60/70/80 覆盖）

> **已过时：** 下表为早期「宁松勿紧」85/80/75/70 版本。**现行唯一表**见上文 `REGIME_RADAR`：**50/60/70/80**（`9bae848`）。

| 档位 | 历史激活（勿再用） | 步进 | 呼吸 |
|------|-------------------|------|------|
| R1 | ~~85%~~ → **现 50%** | 35% | 1.0 ATR |
| R2 | ~~80%~~ → **现 60%** | 30% | 0.8 ATR |
| R3 | ~~75%~~ → **现 70%** | 25% | 0.65 ATR |
| R4 | ~~70%~~ → **现 80%** | 20% | 0.5 ATR |
...
- **仓位**：唯一公式 `权益×risk_pct/|price−tv_sl|`，再与杠杆限制取 min × `qty_ratio`（**无** 50k 硬顶）；OPEN/ADD 同式；删除 REGIME_MARGIN
