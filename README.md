...
[![Exchange](https://img.shields.io/badge/Exchange-Binance%20%7C%20OKX%20%7C%20Gate-yellow)]()
[![Domain](https://img.shields.io/badge/Production-twinstar.pro-green)]()

多用户 **AI 量化决策引擎 SaaS** 平台。用户侧呈现为 AI 托管叙事；底层为成熟的 **TradingView 信号 → 平台广播 → 多交易所 U 本位合约独立执行** 架构。

> **当前阶段（Gemini 多用户）支持 Binance / OKX / Gate.io U 本位永续，以及 DeepCoin SWAP 内测；默认 ETH 永续 × `20×` 杠杆。**  
> Bybit 等列为二期。  
...
| **对内实现** | TradingView 策略发信号 → VPS 统一接收 → 多用户、多交易所并发下单 |
| **资金隔离** | 每位用户独立交易所 API Key，资金与仓位互不影响 |
| **收费模型** | 周期净盈利 × 25% 作为 **AI 绩效服务费**；推广人从服务费池分润 |
| **结算周期** | **30 天/月**；无盈利或仍有持仓宽限至 35 天；须全平仓后结算 |
...
Gemini **读取用户绑定的交易所 API**（Binance / OKX / Gate / DeepCoin），不接入 legacy 单账户 Deepcoin 多用户托管。
...
| 项目 | 默认比例 | 说明 |
|------|----------|------|
| AI 绩效服务费 | 25% | 用户周期 **净盈利** 的 25%（`.env` → `PLATFORM_FEE_RATE`） |
| 一级推广奖励 | 10% | 下级周期 **净盈利** 的 10%，从用户所付绩效费中划出（`REFERRAL_L1_RATE`） |
| 二级推广奖励 | 5% | 下下级周期 **净盈利** 的 5%，从绩效费中划出（`REFERRAL_L2_RATE`） |
| 平台净留存 | 10% | 例：盈利 $1000 → 用户付 $250，L1 $100 + L2 $50，平台得 $100 |

### 完整资金闭环

```
TradingView 信号 → 用户交易所账户交易 → 周期结束且全平仓
    → 系统生成绩效结算账单（25% 服务费）
...
| **用户交易所账户** | 各自 API Key 独立下单（Binance / OKX / Gate / DeepCoin），密钥 Fernet 加密存储 |
...
│   │   │   ├── okx_client.py       # OKX SWAP REST（Binance 兼容接口）
│   │   │   ├── gate_client.py      # Gate.io USDT 永续 REST
│   │   │   ├── deepcoin_client.py  # DeepCoin SWAP REST
...
1. 用户绑定 API 或上次确认结算后，开始 **30 天月度周期**（`SETTLEMENT_PRIMARY_DAYS`）
2. 到期扫描：若 **无净盈利** 或 **仍有持仓** → 宽限至 **35 天**（`SETTLEMENT_EXTENDED_DAYS`）
...
| API 管理 | `/api` | 绑定 Binance / OKX / Gate / DeepCoin 合约 API |
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `SYMBOL` | ETHUSDT | 币安交易对 |
| `LEVERAGE` | 20 | 币安杠杆 |
| `OKX_SYMBOL` | ETH-USDT-SWAP | OKX 永续 |
| `OKX_LEVERAGE` | 20 | OKX 杠杆 |
| `GATE_SYMBOL` | ETH_USDT | Gate.io 永续 |
| `GATE_LEVERAGE` | 20 | Gate 杠杆 |
| `DEEPCOIN_SYMBOL` | ETH-USDT-SWAP | DeepCoin 永续 |
| `DEEPCOIN_LEVERAGE` | 20 | DeepCoin 杠杆 |
...
| `SETTLEMENT_PRIMARY_DAYS` | 30 | 月度主结算周期 |
| `SETTLEMENT_EXTENDED_DAYS` | 35 | 宽限周期（+5 天） |
...
- [x] 币安 U 本位多用户执行引擎（ETHUSDT 20×）
- [x] **OKX / Gate.io 多用户 API + PositionSupervisor 执行**
- [x] DeepCoin SWAP 内测通道（独立 Supervisor）
- [x] VPS 智能接管 + `radar_context` 交叉验证
- [x] 止盈对齐扫描 + 智能撤销重挂（防重启叠单）
- [x] **实盘核实 TradeLog + 全站/用户/推广者明细 UI**
- [x] **钉钉关键动作过滤（过程类仅写日志）**
- [x] 推广者 L1/L2 下级日志与账户 API
- [x] 绩效结算 月度（30/35 天）+ 结算门禁
