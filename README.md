...
| 算仓 | `min(权益×20%/\|价−initialStop\|, 权益×5/价, TV.qty)`；`initialStop`=VPS `entry±1.5×ATR`（TV `stop_loss` 仅日志） |
...
# 关键模块
sizing: backend/app/core/tv_entry_sizing.py          # RISK20 + TV.qty
...
3. **下单数量每次独立计算** — 只依赖余额、开仓价、`initialStop`、TV.qty  
...
风险资金 = 合约权益 × 0.20
名义上限 = 合约权益 × 5
initialStop = 开仓价 ± 1.5 × VPS_ATR     # 开仓前由 market_engine 计算
理论数量 = min(风险资金 / |开仓价 − initialStop|, 名义上限 / 开仓价)
最终数量 = floor(min(理论数量, TV.qty) / 步长) × 步长
```

| 规则 | 说明 |
|------|------|
| TV `stop_loss` | **不参与**算仓与挂止损，仅日志参考 |
| 缺 `initialStop`（ATR 不可用）或缺 `TV.qty` | **拒开仓** |
...
|------|------|
| `qty` | **算仓上限**（必填有效） |
| `qty1` / `qty2` | TP 挂单量（可选；缺省用 30/30 比例） |
| `tp1` / `tp2` | TP 限价价格 |
| `tp3` | 可传但 **VPS 不挂** |
| `stop_loss` | **仅日志**；止损用 VPS `initialStop` |
| `atr` / `adx` | **不读**；行情引擎自算 |
...
| `initialStop` | `entry ± 1.5×ATR`，阶梯基准 |
| `currentStop` / `current_sl` | 当前止损，只朝盈利方向移 |
| `best_price` | 持仓极值 |
| `breakevenPhase` | 是否阶段二（只升不降） |
| `remainingQtyPct` | TP 后剩余比例 |
| `schema_version` | ≥2；旧雷达 schema → 告警暂停 |

### 阶段一（保本前）

```
step_count = floor(|price − entry| / (0.75 × ATR))
step_stop  = initialStop ± step_count × 0.4 × ATR
candidate  = max/min(currentStop, step_stop)   # 只朝盈利

若浮盈 ≥ 1.35×ATR（约 TP1）：candidate 不低于 entry±0.5×ATR
若浮盈 ≥ 2.5×ATR（约 TP2）：candidate 不低于 entry±1.5×ATR
若浮盈 ≥ 3.0×ATR → 进入阶段二
```
...
```
trail_dist = trail_distance(adx) × ATR   # 15→1.2 … 35→2.5 线性插值
...
- 改单/下单失败 → **`HARD_SL_FAIL_ABORT`**：重试后仍失败则告警/可撤仓禁裸奔（执行层容错，非自主策略平仓）
...
| `missing_stop` / `missing_tv_qty` | 行情 ATR 是否就绪；TV 是否带 `qty` |
