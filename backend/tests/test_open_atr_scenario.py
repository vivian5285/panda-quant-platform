"""Open ATR scenario: VPS 1h preferred; TP3 always hung; hard never rewritten."""
...
def test_temp_tv_stop_ignores_atr_widen():
    ...
    # Distance from TV only: |1897.03-1912.18|*1.2
    expect = entry + abs(entry - tv_sl) * TEMP_TV_STOP_BUFFER
    assert abs(hard - expect) < 1e-9


def test_placeable_tp_levels_always_all_three():
...
def test_resolve_open_atr_always_tp3_active():
...
def test_upgrade_keeps_tp3_and_never_retreats_long():
    class Sup:
        user_id = 6
        initial_atr = InitialAtrDescriptor()
    ...
    assert s.tp3_limit_active is True
    assert s._cancelled is None  # must NOT cancel TP3
...
def test_supervisor_placeable_always_all():
    s = MagicMock()
...
def test_mutex_cancel_tp3_on_radar_exit():
    ...
    class H(AdverseRadarMixin):
        ...
        def _collect_tp_limit_orders(self):
            return [{"price": 1930.0, "orderId": 99}]
    ...
    assert any(a[1] == "TP3_RADAR_MUTEX" for a in h.alerts)
