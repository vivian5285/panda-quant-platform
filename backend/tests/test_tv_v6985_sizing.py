...
def test_regime_margin_coeff_dual_symbol_spec():
    assert regime_margin_coeff(1) == pytest.approx(0.08)
    assert regime_margin_coeff(2) == pytest.approx(0.14)
    assert regime_margin_coeff(3) == pytest.approx(0.20)
    assert regime_margin_coeff(4) == pytest.approx(0.26)


@pytest.mark.parametrize(
    "regime,price,expected_qty",
    [
        (1, 2000.0, 1.0),     # 1000×8%×25 / 2000
        (2, 2000.0, 1.75),    # 1000×14%×25 / 2000
        (3, 2000.0, 2.5),     # 1000×20%×25 / 2000
        (4, 2000.0, 3.25),    # 1000×26%×25 / 2000
    ],
)
...
def test_resolve_open_uses_margin_coeff():
    ...
    assert meta.get("margin_coeff") == pytest.approx(0.08)
