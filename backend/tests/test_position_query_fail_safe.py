...
def test_recover_missed_flat_skips_when_query_fails(tmp_path, monkeypatch):
    ...
    sup._alert = _alert
    # Non-rate-limit API failure still alerts
    sup.position_manager.get_position = MagicMock(
        side_effect=ExchangeTransientError("timeout", exchange="binance", code=-1001)
    )
...
def test_rate_limit_query_fail_skips_dingtalk(tmp_path, monkeypatch):
    """-1003 must not spam EXCHANGE_QUERY_FAIL DingTalk/TG."""
    ...
    sup.current_side = "LONG"
    alerts = []
    sup._alert = lambda *a, **k: alerts.append((a, k))
    PositionSupervisor._handle_position_query_failure(
        sup, ExchangeTransientError("banned", exchange="binance", code=-1003)
    )
    assert getattr(sup, "_position_query_degraded", False) is True
    assert not any("EXCHANGE_QUERY_FAIL" in str(x) for x in alerts)
    reset_for_tests()
