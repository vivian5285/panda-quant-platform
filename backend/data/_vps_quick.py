...
# Last events since 15:30
o,e = run(f'cd {PROJ} && docker compose logs --since "2026-07-28T15:30:00" backend 2>&1')
safe = ''.join(c for c in o if ord(c) < 128 or c == '\n')
lines = safe.split('\n')
events = [l for l in lines if any(kw in l for kw in ['UserEvent', 'ERROR', 'WARNING', 'OPEN', 'CLOSE', 'TP', 'FILLED', 'RADAR', 'DingTalk', 'startup'])]
for e in events[-20:]:
    print(e[:180])
...
with open(r"C:\Users\Administrator\AppData\Local\Temp\_vps_quick.py", "w", encoding="utf-8") as f:
    f.write(script)

result = subprocess.run(
    ["py", r"C:\Users\Administrator\AppData\Local\Temp\_vps_quick.py"],
