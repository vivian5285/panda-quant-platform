"""Check Gemini live trading state on VPS - using docker logs only."""
import paramiko

client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect('187.77.130.144', username='root', password="w'tFzgg2vPZ0D,Z;", timeout=30)

def run(cmd, timeout=120):
    i,o,e = client.exec_command(cmd, timeout=timeout)
    return o.read().decode('utf-8', errors='replace'), e.read().decode('utf-8', errors='replace')

PROJ = '/home/panda/panda-quant-platform'

# Health
o,e = run('curl -s http://localhost:8000/api/health')
safe = ''.join(c for c in o if ord(c) < 128 or c == '\n')
print('=== HEALTH ===')
print(safe.strip())

# VPS time
o,e = run('date')
print('\n=== VPS TIME ===')
print(o.strip())

# Recent events (last 3 hours)
o,e = run(f'cd {PROJ} && docker compose logs --since "3 hours ago" backend 2>&1')
safe = ''.join(c for c in o if ord(c) < 128 or c == '\n')
lines = safe.split('\n')
events = [l for l in lines if any(kw in l for kw in [
    'UserEvent', '[OPEN]', '[CLOSE]', 'TP', 'FILLED', 'DingTalk',
    'signal', 'XAU', 'ETH', 'position', 'Telegram', 'SHORT', 'LONG',
    'webhook', 'RADAR', 'ERROR', 'WARNING', 'startup', 'dispatch',
    'SIGNAL_RECV', 'DISPATCH', 'reentry', 'market', 'limit', 'pending_tag'
])]
print('\n=== RECENT TRADING EVENTS (last 3h) ===')
for e in events[-80:]:
    print(e[:200])

client.close()
print('\nDONE')
