    ...
    def _reconcile_tp_defenses_on_startup(
        ...
        for attempt in range(STARTUP_ORDER_FETCH_RETRIES):
            ...
            if attempt == 0:
                ...
                # ROOT CAUSE FIX: When live_qty ≈ anchor (no TP1 fill) but TP1 was excluded by price
                ...
                if (
                    ...
                    no_fill_evidence = (
                        initial_qty_val == 0
                        or live_qty >= anchor * 0.95
                    )
                    if len(tv_tps) > 0:
                        _tp1 = float(tv_tps[0])
                        tp1_price = _tp1 if _tp1 > 0 else 0.0
                    else:
                        tp1_price = 0.0
                    tp1_excluded = (
                        audit.get("expected", 0) == 1
                        and tp1_price > 0
