...
DEEPCOIN_SUPERVISOR_VERSION = "v13.91.7-hedge-sterile"
...
class DeepcoinPositionSupervisor(PositionCapGuardMixin, AdverseRadarMixin, StartupReconcileMixin):
    ...
    def _resolve_entry_qty(self, curr_px: float) -> tuple[int, dict]:
        ...
        # §25 Fix (2026-08-03): 硬性持仓上限检查（DeepCoin版，合约数量）
        # 本金 × 20% × 5倍杠杆 = 100% 名义敞口上限（张数 = 名义敞口 / face_value / price）
    ...
    def _place_tv_entry_order(self, action: str, qty: float, limit_px: float, *, place_limit_fallback: bool = False) -> dict:
        ...
        # ⚠️ 限价回退已禁用（place_limit_fallback=False），此处不再执行
        # market_err 时直接返回失败，由调用方处理重试
        logger.warning(
            "DeepCoin 市价开仓失败且不接受限价回退: %s | symbol=%s qty=%s",
            market_err, self.symbol, qty,
        )
        return {
            **market_meta,
            "status": "market_failed_no_fallback",
            "filled": False,
    ...
    def _protect_and_monitor(self, qty, entry_price):
        ...
        if verified:
            ...
            try:
                scenario_detail = self._resolve_and_apply_open_atr_scenario(entry) or {}
            ...
            if not scenario_detail.get("ok"):
                ...
                try:
                    ...
                    if float(getattr(self, "current_sl", 0) or 0) <= 0:
                        self._init_breathing_on_open(entry, atr=float(DEFAULT_ATR))
                    scenario_detail = {
                        **scenario_detail,
                        "ok": True,
                        "degraded": True,
