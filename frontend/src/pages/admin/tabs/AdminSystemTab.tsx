...
export default function AdminSystemTab() {
  const {
    ...
    webhookSettings, webhookSettingsLoadError, webhookSecretDraft, setWebhookSecretDraft, saveWebhookSettings, clearWebhookSettings,
  } = useAdmin()
  const [expandedLog, setExpandedLog] = useState<number | null>(null)

  const defaultWebhookUrl = 'https://twinstar.pro/gemini/webhook'
  ...
  return (
    <>
      ...
      <GlassCard className="p-6 section-mb-lg webhook-settings-card">
        ...
        <p className={`text-xs section-mb-sm ${webhookStatusClass}`}>
          {webhookStatusText()}
        </p>
        <div className="section-mb-sm">
          <p className="text-sm-strong section-mb-xs">{t('admin.webhookUrlTitle')}</p>
          <code className="webhook-url-display">{displayWebhookUrl}</code>
        ...
        <form onSubmit={saveWebhookSettings} className="form-stack section-mb-md">
          ...
          <div className="flex-gap-sm flex-wrap">
            ...
            {webhookSettings?.configured && webhookSettings.source === 'runtime' && (
  ...
  "tv_tp3": 3800
      ...
      {productionReady === false && (
        <GlassCard className="p-6 section-mb-lg production-checklist-card">
          ...
          {secWarnings.some((w: string) => w.includes('WEBHOOK_SECRET')) && (
            <p className="text-sm section-mt-sm">
              <a href="#webhook-settings" className="text-link">{t('admin.webhookFixLink')}</a>
            </p>
          )}
        </GlassCard>
      )}
