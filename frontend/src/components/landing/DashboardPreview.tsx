...
export default function DashboardPreview({ slide }: Props) {
  const t = useI18n(s => s.t)

  if (slide === 'dashboard') {
    return (
      <div className="product-preview product-preview-dashboard">
          ...
          <div className="preview-chart-row">
            <div className="preview-chart-panel glass">
              ...
              <svg viewBox="0 0 400 100" preserveAspectRatio="none" className="preview-line-chart">
                ...
                <path d={CHART_AREA} fill="url(#previewGrad)" />
                <path d={CHART_PATH} fill="none" stroke="#00B050" strokeWidth="2" />
              </svg>
            </div>
            <div className="preview-chart-panel glass preview-pie-panel">
              <small>{t('dashboard.pnlSource')}</small>
              <div className="preview-pie">
                <div className="preview-pie-ring" />
                <span>R3</span>
              </div>
            </div>
          </div>
          <div className="preview-ticker glass">
            {['LONG ETH +$42.10', 'CLOSE BTC +$18.30', 'LONG ETH +$12.00'].map(tx => (
              <span key={tx} className="preview-ticker-item">{tx}</span>
            ))}
          </div>
        </div>
      </div>
  ...
  if (slide === 'trading') {
    return (
      <div className="product-preview product-preview-trading">
        ...
        <div className="preview-position glass">
        ...
        <div className="preview-bars">
          {BAR_HEIGHTS.map((h, i) => (
            <div
              ...
              style={{ height: `${h}%`, background: i % 4 === 1 ? '#EF4444' : '#00B050' }}
            />
          ))}
        </div>
      </div>
