...
5. **算仓铁律（永远）**：合约本金余额 × 20% 风险资金 ∩ 合约本金余额 × 5 名义上限；无折损、无旧 `(×0.20×5)/price` 路径
...
风险资金 = 账户本金(合约余额) × 20%
名义上限 = 账户本金(合约余额) × 5          # 无 0.85 haircut；永远满额 5× 上限
...
# 若 TV.qty 经调整后仍相对 risk∩notional 荒谬（≥50×），忽略 TV 上限，仅 risk∩notional
最终下单数量 = 理论数量，向下取整至交易所精度
# 硬顶：qty×price 不得超过 合约本金×5（防天文数字）
```
...
| 杠杆 | 统一 **5×** |
| 日志 | `adjust_coef`、三候选、`binding`、最终 qty、`tv_qty_ignored_absurd` |
