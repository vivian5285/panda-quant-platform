...
| `stop_loss` | 开仓硬止损：`dist=\|TV.price−SL\|×buffer`，挂单自 **fill** | **是**（永冻至 flat） |
| `tp1` / `tp2` / `tp3` | 始终挂限价，数量固定 **10%/20%/70%**；TP3 与雷达并行互斥 | **否**（限价目标） |

**开仓硬门**：TV `atr` 缺失/≤0/中位数异常 → **拒开仓**。`stop_loss` 缺失或距&lt;5 ticks → **拒开仓**。场景一成功后 `initial_atr` 改为 VPS 真实 1h ATR（仅雷达，不改硬/不撤 TP3）。
...
1. 用 fill ± TV距×buffer 挂**硬止损**（禁止裸奔；永冻）
2. 挂 TP1/TP2/TP3（TV 价 · **10%/20%/70%**）
3. 立即拉交易所原生 **1h** K 线算真实 ATR(14) 武装雷达

### 场景一（首选）：VPS 真实 ATR 成功

1. `initial_atr` / `initialStop` = 真实 ATR 重算（雷达轨）
2. **保留 TP3**；硬止损不动
3. 雷达与 TP3 并行，谁先成交撤另一条

### 场景二（降级）：VPS 拉取失败

1. **不暂停**该 symbol 开仓；钉钉仅记录
2. `initial_atr` = TV `atr`；呼吸引擎照常
3. TP3 本来就挂着；tick 持续重试 VPS ATR → 成功则升级雷达 ATR，**不撤 TP3**
...
|------|-----|-----|
| 挂单缓冲 | 0.3 | 0.5 |
| 早保本 | 0.5×ATR（档位递进） | 0.65×ATR（档位递进） |
| 雷达启动 | TP1×50/65/80/95 | 同左 |
| 冷启动（ratio=1.0） | **1.525** | **1.525** |
| phase2 coef | 1.2~2.5 | 1.2~2.5 |
