...
|------|------------|------------------------|
| `price` | 开仓参考价 | **否**（算仓用） |
| `atr` | **冻结为 `initial_atr`**（呼吸止损基准；**唯一来源**） | **是**（倍数基准，非直接挂单价） |
| `qty` / `qty1` / `qty2` / `qty3` | **完全忽略**（可缺省；不算仓、不挂 TP 量） | **否** |
| `stop_loss` | 仅日志/归因参考；**不挂止损、不反推开仓 atr** | **否** |
| `tp1` / `tp2` | TP1/TP2 限价止盈**价格** | **否** |
| `tp3` | **不使用** | 不适用 |

**拍板（2026-07-23）：`initial_atr` 继续沿用 TV webhook `atr` 字段。** 异常（缺失/0/负/中位数异常）一律拒开仓，不回退 VPS K 线。
